...
class RoutesSearcher {
    ...
    async generateRouteProto(route) {
        ...
        lines.push(`service Mono{rpc Call(M${this.idMap.get(this.reqFullName)}) returns (M${this.idMap.get(this.respFullName)});}`);
                    ...
                    if (err) {
                        ...
                        fs.writeFile(path, proto, (err) => {
                            if (err) {
                                ...
                                const stream = clang_format_1.default({ path }, 'utf-8', '{Language: Proto,IndentWidth: 4,BasedOnStyle: Google}', function () { });
                                const bufs = [];
    ...
    generateFieldProto(field, oneof = false) {
        ...
        if (!resolvedType) {
            ...
            const keyType = field.keyType || `M${this.idMap.get(resolvedKeyType.fullName)}`;
